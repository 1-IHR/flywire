...
Two guard conditions heuristically skip a triplet early if the compatibility graph has density > 0.8 and more than 50 nodes. This combination indicates the signature filtering was not discriminative enough for that triplet and Bron-Kerbosch would not finish within the timeout. Neither condition triggered on the winning triplet. 
...
Scoring. Each triplet is scored as N * (1 + edge_density) on the largest weakly connected component of the matched node set, requiring N >= 3 and edges >= N. This penalises isolated matched pairs that form no coherent circuit and selects the best triplet across all 10 combinations.
Assumptions
...
MAX_EDGES_PER_SIG
4
Keeps compatibility graph construction tractable (O(n^2) at n<=3000)
MAX_EDGE_TRIPLES
3000
Hard cap; degree-sorted so best candidates are retained
KCORE_K
2
Removes nodes that cannot belong to any clique of size >= 3
TIMEOUT_S
600
Sufficient for Bron-Kerbosch to exhaust the pruned search space
...
Final result
Best triplet: BANC x FAFB x MCNS Matched neurons: 32 Induced directed edges: 55 Isomorphism violations: 0
